 ...
 */
#define SPI3_FIFO_FILL_LIMIT 0x4

//Check to make sure that the FIFO limit does not exceed the maximum allowed limit of 8
#if (SPI3_FIFO_FILL_LIMIT > 4)

    #define SPI3_FIFO_FILL_LIMIT 4
...
/**
  ...
  @Example
    ...
    do
    {
        total  = SPI3_Exchange16bitBuffer( &myWriteBuffer[total], MY_BUFFER_SIZE - total, &myWriteBuffer[total]);
